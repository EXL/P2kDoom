 ...
 *
 * EXL, 04-Aug-2025:
 *      SDL3 platfrom implementation based on i_vvga13.c blitter by FrenkelS.
 *
 *-----------------------------------------------------------------------------*/
...
static SDL_Color *palette_doom;
static SDL_Palette *palette_sdl;
...
static void I_UploadNewPalette(int8_t pal)
{
	...
	if (palette_lump != NULL)
	{
		...
		for(int p = 0; p < 256; p++)
		{
			palette_doom[p].r = palette[3*p];
			palette_doom[p].g = palette[(3*p)+1];
			palette_doom[p].b = palette[(3*p)+2];
			palette_doom[p].a = 0xFF;
		}
		SDL_SetPaletteColors(palette_sdl, palette_doom, 0, 256);
...
void I_InitGraphicsHardwareSpecificCode(void)
{
	...
	video = SDL_CreateSurface(screen_width, screen_height, SDL_PIXELFORMAT_RGB565);
	if (video == NULL) {
		SDL_Log("SDL_CreateSurface (video) failed: %s", SDL_GetError());
		return;
	}

	surface = SDL_CreateSurface(screen_width, screen_height, SDL_PIXELFORMAT_INDEX8);
	if (surface == NULL) {
		SDL_Log("SDL_CreateSurface (surface) failed: %s", SDL_GetError());
		return;
	}
	palette_sdl = SDL_CreateSurfacePalette(surface);
	if (palette_sdl == NULL) {
		SDL_Log("SDL_CreateSurfacePalette (palette_sdl) failed: %s", SDL_GetError());
		return;
	}
	...
	palette_doom = (SDL_Color *) malloc(sizeof(SDL_Color) * 256);
...
void I_ShutdownGraphics(void)
{
	...
	SDL_DestroySurface(video);

	/*
	 *  EXL, 04-Aug-2025: Do not need to destory it, see:
	 *  https://wiki.libsdl.org/SDL3/SDL_CreateSurfacePalette
	 */

//	SDL_DestroyPalette(palette_sdl);
	...
	while (SDL_PollEvent(&event) != 0) {
		switch (event.type) {
			...
			case SDL_EVENT_KEY_DOWN:
				ev.type = (event.type == SDL_EVENT_KEY_UP) ? ev_keyup : ev_keydown;
