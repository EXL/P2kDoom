...
ldrElf *_start(WCHAR *uri, WCHAR *arguments) {
	...
	if (ldrIsLoaded(g_app_name)) {
		cprint("P2kDoom8088: Error! Application has already been loaded!\n");
		return NULL;
	}
...
static UINT32 ATI_Driver_Start(APPLICATION_T *app) {
	...
	appi->ahi.bitmap.image = suAllocMem(SCREENWIDTH * SCREENHEIGHT, NULL);
...
static uint32_t *indextable = NULL;
...
//    ytable2 = malloc(sizeof(uint16_t) * video_h);

    // For CW 90° rotation:
    // Output: (x, y) in [0, video_w) x [0, video_h)
    // Output buffer: doomFB16bpp[y * video_w + x]
    // Source: (src_x, src_y) = (scaled and rotated)
    // After rotation, dest(x, y) = src(y, SCREENWIDTH-1-x)
    // But we want to scale too, so:
    // src_x = (y * SCREENWIDTH) / video_h      // scale y to src_x
    // src_y = ((video_w - 1 - x) * SCREENHEIGHT) / video_w  // scale "reverse x" to src_y

    for (i = 0; i < video_h; i++) {
        // x in output becomes y in source
        // So for each y in output, precompute src_x offset for all x
        // ytable1[i] is the offset for src_x in the source buffer
        ytable[i] = (i * SCREENWIDTH) / video_h;  // src_x
//        ytable2[i] = i * video_w;                 // dest buffer offset for y
    }
    for (i = 0; i < video_w; i++) {
